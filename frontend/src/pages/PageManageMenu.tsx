        ...
        <div className="grid grid-cols-5 grid-rows-8 h-screen bg-mealshub-cream">
            ...
            <div className="col-span-4">
                <div className="ms-20">
                    ...
                    <div className="row-span-6 mt-6 mb-9 py-7 w-11/12 bg-white rounded-3xl" style={{
                        height: '804px',
                    }}>
                        <h2 className="text-mealshub-red text-4xl font-bold ps-9">Menu</h2>
                        <div className="grid grid-cols-4 gap-6 h-4/5 my-6 mx-9 justify-items-center overflow-y-auto overflow-x-hidden ">
                            {data.map((menus) => (
                                <div>
                                    <MenuCard image={menus.image} stock={menus.stock} name={menus.name} onEditClick={handleEditClick} onDeleteClick={handleDeleteClick} />
                                    {isEditFormVisible && <EditForm onClose={handleCloseEditForm} />}
                                    {isDeletePopUpVisible && <DeletePopUp onClose={handleCloseDeletePopUp} />}
                                </div>
                            ))}
                        </div>
                        <div className="my-6 flex flex-col items-center justify-center">
                            <AddButton onClick={handleAddMenuClick} />
                            {showAddMenu && <AddForm onClose={handleCloseAddMenu} />}
                        </div>
                    </div>
                </div>
            </div>
