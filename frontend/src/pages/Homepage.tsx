...
export default function Homepage() {
  ...
  useEffect(() => {
    const filtered = dataTenant.filter((tenant) =>
      tenant.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
    setFilteredData(filtered);
  }, [searchTerm, dataTenant]);

  const data = filteredData.map((tenant: Tenant) => {
    const products: Product[] = dataProduct.filter(
      (product: Product) => product.id_tenant === tenant.id
    );
    return {
      id: tenant.id,
      image: tenant.image,
      ...
      <div className="col-span-4">
        <div className="ms-20">
          ...
          <div className="row-span-1 mt-6 py-3 w-11/12">
            <Search onChange={handleSearch} onKeyDown={handleEnter} />
          </div>
          <div className="row-span-6 mt-6 mb-9 py-7 w-11/12 bg-white rounded-3xl">
            <h2 className="text-mealshub-red text-3xl font-bold ps-9 mb-10">Recommended Tenants</h2>
            <div className="grid grid-cols-3 gap-12 my-6 mx-9 justify-items-center">
