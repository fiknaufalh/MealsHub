...
export default function ShoppingCart() {
    ...
    const handleCheckout = async () => {
        await Axios.post("http://localhost:8000/orders", {
            ...
            id_table: tableid,
            id_tenant: getTenantId(cartItems[0]),
        });

        const orderId = Math.max(...(await Axios.get("http://localhost:8000/orders")).data.data.map((order: any) => order.id));

        const arrayOrderProduct = cartItems.map((cartItem) => {
            return {
                id_order: orderId,
                id_product: cartItem.id,
                num_product: cartItem.quantity,
            };
        });

        console.log(arrayOrderProduct);

        await Axios.post("http://localhost:8000/orderproduct/bulk", arrayOrderProduct);
                ...
                <div className="ms-20">
                    ...
                    <div className="row-span-7 mt-6 mb-9 py-12 w-11/12 bg-white rounded-3xl">
                        <div className="">
                            ...
                            {cartItems.length > 0 && (
                                <>
                                    <div className="flex py-10 mx-16 border-t border-mealshub-red justify-between items-center">
                                    ...
                                    <div className="flex mx-16 justify-center">
                                        <button onClick={handleCheckout} className="w-full text-white bg-mealshub-red font-bold text-lg rounded-full px-5 py-2.5 text-center shadow-xl">
                                            Checkout
                                        </button>
                                    </div>
                                </>
