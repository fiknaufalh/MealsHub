...
export default function TenantCard({ data }: { data: TenantCardProps[] }) {
    const tenantlist = data.map(
        ({
            ...
            const lowestpriceidr = lowestprice
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ".");
            const highestpriceidr = highestprice
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ".");
                    ...
                    <div className="w-72 h-80 bg-white rounded-3xl shadow-xl">
                        ...
                        <div className="mt-3 px-4">
                            ...
                            <div className="flex items-center">
                                <span>
                                    <svg
                                        className="w-6 h-6 text-mealshub-orange"
                                        aria-hidden="true"
                                ...
                                <span className="px-1.5 mr-1.5 font-light">
                                    {rating}
                                </span>
                                <div className="border-r h-4 border-gray-300"></div>
                                <span className="px-1.5 ml-1.5 font-light">
                            ...
                            <div className="pl-1.5 mt-2 flex items-center">
                                ...
                                <span className="px-3 font-light">
                                    Rp.{lowestpriceidr} - Rp.{highestpriceidr}
                                </span>
                            </div>
                        </div>
                    </div>
